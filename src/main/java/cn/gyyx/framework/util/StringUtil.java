...
//            nWeek = calendar.get(Calendar.DAY_OF_WEEK);
     ...
     * @throws Exception
     */
    public static int indexOfCount(String content, String seach, int count) throws Exception
    {
        String startText = "";
        int index = -1;
...
//		java.util.regex.Matcher m_html;
    ...
    /**
     * 去除字符串中html代码,由于新闻中的特殊要求保留img标签
     * @param inputString
     * @return
     */
    public static String htmlToTextIgnoreImg(String inputString, String mode)
    {
        String htmlStr = inputString; // 含html标签的字符串
        String textStr = "";
        java.util.regex.Pattern p_script;
...
//		java.util.regex.Matcher m_html;
        ...
        try {
            ...
            // htmlStr = m_html.replaceAll(""); //过滤html标签
            // 过滤html标签
            htmlStr = htmlStr.replaceAll("<[a-hj-zA-HJ-Z]+[1-9]?[^><]*>", "")
                    .replaceAll("</[a-hj-zA-HJ-Z]+[1-9]?[^><]*>", "");
            htmlStr = htmlStr.replaceAll("\t|\r|\n", "");// 去除字符串中的空格,回车,换行符,制表符
            htmlStr = htmlStr.replaceAll("&nbsp;", " ");// 去除字符串中的空格
            htmlStr = replaceHtmlNote(htmlStr);
            textStr = htmlStr;
            textStr = textStr.replaceAll("<input", "<img").replaceAll("type=\"image\"", "");

        } catch (Exception e) {
            System.err.println("Html2Text: " + e.getMessage());
        }

        return replaceHttpImg(textStr, mode);// 返回文本字符串
    }
     ...
     * @return
     */
    public static String replaceHttpImg(String content, String mode)
    {
        while(true)
        {
...
//                        +srcurl;
                ...
                if(mode.equals("10.2"))
                {
                    content = content.replace(img, "<pic>"+url.replaceAll("/original/", "/midsize/")+"</pic>");
                }
                else
                {
                    content = content.replace(img, "<pic>"+url.replaceAll("/original/", "/small/")+"</pic>");
                }
            }
            else
            {
...
//		}
    ...
    public static synchronized String create10RandomNumByTimes() throws Exception {
        Thread.sleep(1);
    ...
    public static boolean isEmpty(String... arg) {
        for(String s : arg){
            if (StringUtil.isNull(s).trim().equals("")) {
                return true;
            }
        }
        return false;
...
//		System.out.println(getShowmobile("18611189867", "18611189867"));;
        System.out.println(replaceHtmlNote("<!->1234<!--EndFragment-->5678<!---->"));
    }
}
